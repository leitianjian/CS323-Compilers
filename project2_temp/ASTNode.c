...
ASTNode *new_node(char* type, char* value, int terminal, int row)
{
    ASTNode *node = init();
    node->type = type;
    node->value = value;
...
{
    return new_node(type, strcopy(value), 1, 0);
}

ASTNode *newNode(char* type, int row)
{
    return new_node(type, NULL, 0, row);
}

ASTNode *newKWLeaf(char* type)
{
    return new_node(type, NULL, 2, 0);
}
